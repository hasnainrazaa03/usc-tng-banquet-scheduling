...
export function RoleSlot({
  ...
  const open = count - assigned.length;
...
export function ShiftCard({
  ...
  return (
    <div className={`rounded-lg border bg-white overflow-hidden ${isUnderfilled ? "border-red-200" : "border-ink/15"}`}>
      <div className="px-2 py-1.5 bg-canvas-soft/60 border-b border-ink/10">
        ...
        <div className="text-[10px] mt-0.5">
          <span className={isUnderfilled ? "text-red-700 font-semibold" : "text-emerald-700"}>
            {totalAssigned}/{totalReq} staffed
          </span>
        </div>
      </div>
